...
return new class() extends Migration
{
    ...
    private function createAncestorsTable(): void
    {
        ...
        $ancestorMorphType = MorphType::tryFrom(Config::get('ancestry.ancestor_morph_type', 'string')) ?? MorphType::String;
        $descendantMorphType = MorphType::tryFrom(Config::get('ancestry.descendant_morph_type', 'string')) ?? MorphType::String;
        ...
        $schema->create($tableName, function (Blueprint $table) use ($primaryKeyType, $ancestorMorphType, $descendantMorphType): void {
            ...
            match ($ancestorMorphType) {
                MorphType::UUID => $table->uuidMorphs('ancestor'),
                MorphType::ULID => $table->ulidMorphs('ancestor'),
                MorphType::Numeric => $table->numericMorphs('ancestor'),
                MorphType::String => $table->morphs('ancestor'),
            };

            // Descendant polymorphic relationship
            match ($descendantMorphType) {
                MorphType::UUID => $table->uuidMorphs('descendant'),
                MorphType::ULID => $table->ulidMorphs('descendant'),
                MorphType::Numeric => $table->numericMorphs('descendant'),
                MorphType::String => $table->morphs('descendant'),
            };
    ...
    private function createAncestorSnapshotsTable(): void
    {
        ...
        $connection = Config::get('ancestry.connection');
        $primaryKeyType = PrimaryKeyType::tryFrom(Config::get('ancestry.primary_key_type', 'id')) ?? PrimaryKeyType::Id;
        $contextMorphType = MorphType::tryFrom(Config::get('ancestry.snapshots.context_morph_type', 'string')) ?? MorphType::String;
